...
class ConfigManager {
    constructor() {
        ...
        if (process.platform === 'darwin') {
            // macOS
            this.configDir = path.join(os.homedir(), 'Library', 'Application Support', 'my-electron-app');
        } else if (process.platform === 'win32') {
            // Windows
            this.configDir = path.join(process.env.APPDATA, 'my-electron-app');
        } else {
            // Linux or others
            this.configDir = path.join(os.homedir(), '.config', 'my-electron-app');
        }
    ...
    validateConfig(config) {
        ...
        const hasAllValues = required.every(field => 
            field in config && config[field] !== ''
        );

        if (!hasAllValues) {
            console.warn('Configuration is incomplete. Please fill in all required fields.');
            return config;
        }
